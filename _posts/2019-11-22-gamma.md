...
Notice that the left-hand side is a cumulative distribution function of the Gamma distribution expressed in terms of $4t$$. Given the derivative relationship between the CDF and PDF, we can obtain the probability distribution function of Gamma by deriving the right-hand side sigma expression with respect to $$t$$.

$$F’(t) = f(t) = \sum_{x = k}_{\infty} \lambda \frac{{\lambda t}^{x - 1}}{(x - 1)!} e^{- \lambda t} - \sum_{x = k}_{\infty} \lambda \frac{{\lambda t}^x}{x!} e^{- \lambda t}$$
...
Returning back to the Gamma distribution, we see that altering the $$\beta$$ also produces a transformative effect on the shape of the Gamma PDF. Specifically, increasing $$\beta$$ causes the graph to extend in the direction of the y-axis, consequently producing a more confident distribution with a higher peak. 
