...
\begin{align}
z_{d/2:d} 
&= x_{d/2:d} \cdot s_\theta(x_{1:d/2}) + t_\theta(x_{1:d/2}) \\
&= x_{d/2:d} \cdot s_\theta(z_{1:d/2}) + t_\theta(z_{1:d/2})
\end{align}
\tag{5}
$$
...
First, we check that the affine coupling layer only transforms the top half of the input.
