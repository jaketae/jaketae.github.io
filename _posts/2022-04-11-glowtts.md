...
Although $\mathbf{J}_{21}$ contains complicated terms, we do not have to consider them when computing $\det \mathbf{J}$: the determinant of a lower triangular matrix is simply the product of its diagonal entries. Hence, $\det \mathbf{J} = \mathbf{J}_{11} \times \mathbf{J}_{22}$, which is computationally tractable. 
