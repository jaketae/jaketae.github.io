...
$$f(\theta \mid k, n) \propto L(k, n \mid \theta) f_\Beta(\theta; \alpha, \beta) \propto \theta^k (1 - \theta)^{n - k} \theta^{\alpha - 1} (1 - \theta)^{\beta - 1}$$
