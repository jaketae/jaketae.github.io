...
Let's start by coming up with a model representation of the likelihood function, which we might recall is the probability of observing a data point given a parameter value $$\theta$$. It is not difficult to see that the best distribution for the likelihood function given the setup of the problem is the Bernoulli distribution, since each coin flip is a Bernoulli trial. Let $$X$$ denote a random variable which can take two discrete values, $${1, 0}$$, where 1 represents a head and 0 represents a tail. Then, the conditional probability of obtaining a head given parameter $$\theta$$, can be expressed as

$$L(X = 1 \mid \theta) = \theta, L(X = 0 \mid \theta) = 1 - \theta$$

where $$L(X = x)$$ denotes a likelihood function that takes a random variable $$X$$ as an argument. We can express this likelihood function more succinctly by collapsing the two equations into one:

$$L(X = x \mid \theta) = \theta^x (1 - \theta)^{1 - x}$$

Notice that, when $$X = 1$$, $$(1 - \theta)^{1 - x}$$ reduces to 1, leaving only the first half of the equation. The reverse happens when $$X = 0$$, which produces the desired result. 

Now that we have derived a likelihood function, we move onto the next component necessary for Bayesian analysis: the prior. Prior 
