...
$$p_1 = p \cdot (1 - p) \cdot (1 - p) \cdot \dots (1 - p) = {\frac{1}{2}}^{10} = 0.0009765625$$
...
But to really calculate how lucky I get for the next 18 months of my life in the military, I need to also take into account the fact that receiving lesser than 5 calls also constitutes a great day. In other words, we need to calculate $$\sum_{n=0}^5 P(X = n)$$, because a day with lesser than five calls should also count towards my luckiness. Of course, we can directly obtain the six data points by plugging in numbers into the equation we derived earlier. But because we are lazy mathematicians, we consider integration, assuming albeit impractically that the number of phone calls is a continuous random variable, *i.e.* it is possible to have 2.1 calls, for instance. Graphically speaking, the area of the shaded region in the figure below is what we are aiming for. We present the code snippet first.
...
Integration is the way to calculate the area beneath the curve. We are not really going into the details of this operation today, because integrating this function requires an understanding of the [gamma distribution], which appears in the code block above as `scipy.special.gamma()`. To give you a quick preview though, this function is essentially a continuous version of the factorial function, whose operation is undefined for non-integers. For now, let's just say that

* I had a once-in-a-hundred lucky day
