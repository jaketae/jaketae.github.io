...
Notice that $$p = 1 - p = \frac12$$ because we assumed the coin was fair. Had it not been fair, we would have different probabilities for $$p_{success}$$ and $$p_{fail}$$, explained by the relationship that $$p_{success} + p_{fail} = 1$$. This is what the binomial PDF is implying: calculating the probability that we get $$k$$ successes in $$n$$ trials requires that we multiply the probability of success $$p$$ $$k$$ times and the probability of failure $$(1 - p)$$ $$n - k$$ times, because those are the number of successful and unsuccessful trials respectively. 
...
<figure>
	...
	<figcaption>Figure 1: Binomial distribution modeling of phone calls</figcaption>
</figure>
...
$$P(X = 5) = \binom{48}{5} \cdot 0.25^5 \cdot 0.75^43 = 1712304 \cdot 0.0009765625 \cdot 0.00000424262 \approx 0.0071$$
...
Another issue with this model is that a Bernoulli trial does not allow for simultaneous successes. Say, for instance, that within one ten-minute block, we got two calls from two different people. However, because Bernoulli trials operate like an on-off switch, with the result either being a success or a failure, it cannot encode more complicated information such as two or three simultaneous successes in one trial. 

These set of complications motivate a new way of modeling phone calls at the PMO. In the next section, we look at an alternate approach to the problem: the [Poisson distribution]. 
