---
title: "How lucky was I?"
last_modified_at: 2019-11-17 9:50:00 +0000
categories:
  - study
tags:
  - math
...
Notice that $$p = 1 - p = \frac12$$ because we assumed the coin was fair. Had it not been fair, we would have different probabilities for $$p_{success}$$ and $$p_{fail}$$, explained by the relationship that $$p_{success} + p_{fail} = 1$$. This is what the binomial PDF is implying: calculating the probability that we get $$k$$ successes in $$n$$ trials requires that we multiply the probability of success $$p$$ by $$k$$ times and the probability of failure $$(1 - p)$$ by $$(n - k)$$ times, because those are the numbers of successful and unsuccessful trials respectively. 
...
Under this assumption, we can also calculate how lucky I was yesterday when I only received five calls by plugging in the appropriate values into the binomial PDF function:
...
Now let's derive the Poisson formula by tweaking the PDF for a binomial distribution. We commence from this expression: 
...
And we have derived the PDF for the Poisson distribution! We can perform a crude sanity check on this function by graphing it and checking that its maximum occurs at $$X = \lambda$$. In this example, we use the numbers we assumed in the PMO phone call example, in which $$\lambda = 12$$.
...
data_poisson = poisson.rvs(mu=12, size=10000)
...
plt.show()
```
The code produces the following histogram. Note that the code invokes the `scipy.stats.poisson.rvs()` function, which is why each execution will create a different histogram. 
...
But to really calculate how lucky I get for the next 18 months of my life in the military, we need to do a bit more: we need to also take into account the fact that receiving lesser than 5 calls on a shift also constitutes a lucky day. In other words, we need to calculate $$\sum_{n=0}^5 P(X = n)$$. We can achieve this quite easily by directly plugging in numbers into the equation we derived earlier to obtain six data points. But because we are lazy mathematicians, we can also consider integration, assuming albeit impractically that the number of phone calls is a continuous random variable, *i.e.* it is possible to have 2.1 calls, for instance. Graphically speaking, the area of the shaded region in the figure below is what we are aiming for. We present the code snippet first.
...
<figure>
	<img src="/assets/images/poisson-pdf.png">
	<figcaption>Figure 3: Poisson Probability Density Function</figcaption>
</figure>

Integration is the way to calculate the area beneath the curve. We are not really going into the details of this operation today, because integrating the Poisson distribution function requires an understanding of the [gamma distribution], which appears in the code block above as `scipy.special.gamma()`. To give you a intuition though, the gamma function is essentially a continuous version of the factorial function, whose operation is undefined for non-integers. The gamma function is defined for all real numbers and is hence continuous; it also satisfies the condition that $$\Gamma(n) = (n - 1) \cdot \Gamma(n - 1)$$, which is a recursive definition of the factorial function. 

For now, let's just say that

* I had a once-in-a-hundred lucky day yesterday
* I still love working and answering calls
* the Poisson distribution is binomial on steroids
* the Gamma distribution resembles the factorial operation
* using bullet points to summarize posts is awesome

If you insist on completing the calculation of my luckiness by integration, that exercise will be left to you, the keen reader, as a mental exercise. What a way to end a blog post. Next time, I'll come back with another post on the gamma distribution, along with some better, creative ideas on how to end blog posts on a humorous note.
