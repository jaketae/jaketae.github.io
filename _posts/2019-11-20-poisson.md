...
Notice that $$p = 1 - p = \frac12$$ because we assumed the coin was fair. Had it not been fair, we would have different probabilities for $$p_{success}$$ and $$p_{fail}$$, explained by the relationship that $$p_{success} + p_{fail} = 1$$. This is what the binomial PDF is implying: calculating the probability that we get $$k$$ successes in $$n$$ trials requires that we multiply the probability of success $$p$$ $$k$$ times and the probability of failure $$(1 - p)$$ by $$n - k$$ times, because those are the number of successful and unsuccessful trials respectively. 
...
$$ \lim\limits_{n \to \infty} \frac {n!}{(n - k)!} \frac{1}{n^k} = \lim\limits_{n \to \infty} \frac{n}{n} \cdot \frac{n - 1}{n} \frac{n - 2}{n} \dots \frac{n - k + 1}{n} = 1$$
